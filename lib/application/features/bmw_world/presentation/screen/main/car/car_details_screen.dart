...
class CarMainInfo extends StatelessWidget {
    ...
    return BlocBuilder<CarDetailsBloc, CarDetailsState>(
      builder: (context, state) {
        ...
        return Row(
          ...
          children: [
            _buildColumn(context, Icons.speed, 'Max Speed', "$maxSpeed km/h",Colors.blue),
            _buildColumn(context, Icons.engineering_outlined, 'Engine', engineSize,Colors.blueAccent),
            _buildColumn(context, Icons.oil_barrel, 'Consumption', consumption,Colors.redAccent),
          ],
        );
      },
    );
  ...
  Column _buildColumn(BuildContext context,  IconData iconData, String title, String details,Color iconColor) {
    return Column(
      children: [
